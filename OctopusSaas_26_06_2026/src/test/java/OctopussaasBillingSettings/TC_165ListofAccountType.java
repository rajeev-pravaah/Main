...
public class TC_165ListofAccountType extends BaseClass80 {
	...
	@Test
	public void TC_165ListofAccountType() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		// Step 11: Switch to iframe → click on Account Type dropdown → fetch all list values
				...
				try {
					...
					if (!dropdowns.isEmpty()) {
						...
						try {
							...
							System.out.println("INFO: Clicked on Account Type dropdown successfully");
						} catch (Exception clickEx) {
							js.executeScript("arguments[0].click();", accountTypeDropdown);
							System.out.println("INFO: Clicked on Account Type dropdown using JS");
						}
						Thread.sleep(1000);

						// Fetch all options from the dropdown using Select class
						...
						System.out.println("INFO: ===== Account Type Dropdown Options =====");

						StringBuilder allOptionsList = new StringBuilder();
						for (int j = 0; j < allOptions.size(); j++) {
							...
							System.out.println("INFO: Option[" + j + "] text='" + optionText + "' value='" + optionValue + "'");
							allOptionsList.append("[").append(j).append("] ").append(optionText).append(" | ");
						}
						System.out.println("INFO: ==========================================");

						if (!allOptions.isEmpty()) {
							utilityclassobject.gettest().log(Status.PASS,
									"Clicked on Account Type dropdown and fetched all " + allOptions.size()
									+ " options successfully from iframe[" + i + "]: " + allOptionsList.toString());
							System.out.println("PASS: Clicked on Account Type dropdown and fetched all " + allOptions.size() + " options successfully");
						} else {
							utilityclassobject.gettest().log(Status.FAIL, "Account Type dropdown has no options");
							System.out.println("FAIL: Account Type dropdown has no options");
						}
		...
		if (!dropdownFound) {
			try {
				...
				Select selectObj = new Select(accountTypeDropdown);
				List<WebElement> allOptions = selectObj.getOptions();
				System.out.println("INFO: Strategy 2 - Total options: " + allOptions.size());
				StringBuilder allOptionsList = new StringBuilder();
				for (int j = 0; j < allOptions.size(); j++) {
					String optionText = allOptions.get(j).getText().trim();
					String optionValue = allOptions.get(j).getAttribute("value");
				...
				if (!allOptions.isEmpty()) {
					utilityclassobject.gettest().log(Status.PASS,
							"Strategy 2 frame(0): Clicked and fetched all " + allOptions.size()
							+ " options from Account Type dropdown: " + allOptionsList.toString());
					System.out.println("PASS: Strategy 2: Clicked and fetched all " + allOptions.size() + " options from Account Type dropdown");
				} else {
					utilityclassobject.gettest().log(Status.FAIL, "Strategy 2: Account Type dropdown has no options");
					System.out.println("FAIL: Strategy 2: Account Type dropdown has no options");
				}
				dropdownFound = true;
			} catch (Exception e2) {
		...
		if (!dropdownFound) {
			try {
				...
				Select selectObj = new Select(accountTypeDropdown);
				List<WebElement> allOptions = selectObj.getOptions();
				System.out.println("INFO: Strategy 3 PageFactory - Total options: " + allOptions.size());
				StringBuilder allOptionsList = new StringBuilder();
				for (int j = 0; j < allOptions.size(); j++) {
					String optionText = allOptions.get(j).getText().trim();
					String optionValue = allOptions.get(j).getAttribute("value");
				...
				if (!allOptions.isEmpty()) {
					utilityclassobject.gettest().log(Status.PASS,
							"Strategy 3 PageFactory: Clicked and fetched all " + allOptions.size()
							+ " options from Account Type dropdown: " + allOptionsList.toString());
					System.out.println("PASS: Strategy 3 PageFactory: Clicked and fetched all " + allOptions.size() + " options from Account Type dropdown");
				} else {
					utilityclassobject.gettest().log(Status.FAIL, "Strategy 3: Account Type dropdown has no options");
					System.out.println("FAIL: Strategy 3: Account Type dropdown has no options");
				}
				dropdownFound = true;
			} catch (Exception e3) {
