...
public class TC_105AutomaticSuspensiondropdownwith1stvalue extends BaseClass80 {
	...
	@Test
	public void TC_105AutomaticSuspensiondropdownwith1stvalue() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		// Fetch 1st value
		if (allOptions != null && !allOptions.isEmpty()) {
			String firstValue = allOptions.get(0).getText().trim();
			System.out.println("1st Value from dropdown: " + firstValue);
			utilityclassobject.gettest().log(Status.INFO, "1st Value from dropdown: " + firstValue);
		} else {
			System.out.println("No options found in the dropdown");
			utilityclassobject.gettest().log(Status.FAIL, "No options found in the dropdown");
		}
	}
}
