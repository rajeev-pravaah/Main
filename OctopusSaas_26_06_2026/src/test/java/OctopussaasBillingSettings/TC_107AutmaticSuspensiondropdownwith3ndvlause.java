...
public class TC_107AutmaticSuspensiondropdownwith3ndvlause extends BaseClass80 {
	...
	@Test
	public void TC_107AutmaticSuspensiondropdownwith3ndvlause() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		// Fetch 3rd value
		if (allOptions != null && allOptions.size() >= 3) {
			String thirdValue = allOptions.get(2).getText().trim();
			System.out.println("3rd Value from dropdown: " + thirdValue);
			utilityclassobject.gettest().log(Status.INFO, "3rd Value from dropdown: " + thirdValue);
		} else {
			System.out.println("3rd option not found in the dropdown");
			utilityclassobject.gettest().log(Status.FAIL, "3rd option not found in the dropdown");
		}
	}
}
