...
public class TC_159InvalidPostalCode  extends BaseClass80{
	...
	@Test
	public void TC_159InvalidPostalCode() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		try {
			...
			bs.getPostalCodeTextfieldinframe().sendKeys("5");
			//click outside anywhere on screen
			actionsCvc.moveByOffset(100, 100).click().perform();
			...
			//check error message for invalid postal code
			 WebElement errorMessage = wait.until(
					 ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(text(),'postal code is invalid') or contains(text(),'postal code is not valid')]")));
			 if (errorMessage.isDisplayed()) {
				 utilityclassobject.gettest().log(Status.PASS, "Error message for invalid postal code is displayed successfully");
				 System.out.println("PASS: Error message for invalid postal code is displayed successfully");
			 } else {
				 utilityclassobject.gettest().log(Status.FAIL, "Error message for invalid postal code is not displayed");
				 System.out.println("FAIL: Error message for invalid postal code is not displayed");
			 }
		...
		// Step 14: Enter 5-digit Postal Code '10001' in Postal Code text field inside iframe
		try {
			driver.switchTo().defaultContent();
			Thread.sleep(1000);

			WebElement postalCodeField = null;

			By[] postalLocators = {
				By.id("postal-code-input"),
				By.xpath("//input[@id='postal-code-input']"),
				By.xpath("//input[@name='postalCode']"),
				By.xpath("//input[@name='postal_code']"),
				By.xpath("//input[@placeholder='ZIP']"),
				By.xpath("//input[@placeholder='ZIP Code']"),
				By.xpath("//input[@placeholder='Postal Code']"),
				By.xpath("//input[@aria-label='Postal code']"),
				By.xpath("//input[contains(@id,'postal') or contains(@name,'postal') or contains(@id,'zip')]")
			};

			int totalFrames = driver.findElements(By.tagName("iframe")).size();
			System.out.println("INFO: Scanning " + totalFrames + " iframe(s) for Postal Code field");

			outerPostal:
			for (int i = 0; i < totalFrames; i++) {
				for (By locator : postalLocators) {
					try {
						driver.switchTo().defaultContent();
						driver.switchTo().frame(i);
						java.util.List<WebElement> found = driver.findElements(locator);
						if (!found.isEmpty()) {
							postalCodeField = found.get(0);
							System.out.println("INFO: Postal Code field found in iframe[" + i + "] using: " + locator);
							break outerPostal;
						}
			...
			if (postalCodeField == null) {
				...
				for (By locator : postalLocators) {
					...
					if (!found.isEmpty()) {
						postalCodeField = found.get(0);
						System.out.println("INFO: Postal Code field found in default content using: " + locator);
						break;
					}
				}
			}
			...
			if (postalCodeField == null)
				throw new Exception("Postal Code field could not be located in any iframe or on the page.");

			// Scroll, unlock restrictions
			((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", postalCodeField);
			js.executeScript("arguments[0].removeAttribute('disabled')", postalCodeField);
			js.executeScript("arguments[0].removeAttribute('readonly')", postalCodeField);
			js.executeScript("arguments[0].removeAttribute('maxlength')", postalCodeField);
			Thread.sleep(300);

			// Click, clear, then paste via clipboard (most reliable for Stripe iframes)
			

			

			// Also fire native React/Angular events to ensure the framework registers the value
			
			Thread.sleep(500);

			String enteredPostal = postalCodeField.getAttribute("value");
			System.out.println("INFO: Postal Code field value after input: " + enteredPostal);
