...
public class TC_184AddnotewithInValidData extends BaseClass80 {
	...
	// 105 random characters (exceeds typical 100-char limit)
	private static final String INVALID_NOTE =
		"AbCdEfGhIjKlMnOpQrStUvWxYz1234567890!@#$%^&*()_+-=[]{}|;':\",./<>?AbCdEfGhIjKlMnOpQrStUvWxYz123456";

	@Test
	public void TC_184AddnotewithInValidData() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		// Step 14: Verify payment card is added and scroll to it
		try {
			WebElement paymentCard = wait.until(
				ExpectedConditions.visibilityOfElementLocated(
					By.xpath("//div[normalize-space(text())='" + acname + "'] | //p[normalize-space(text())='" + acname + "']")));
		...
		// Step 15: Click on the note text field inside the payment card and enter 105 invalid characters
		System.out.println("INFO: Invalid note data (105 chars): " + INVALID_NOTE);
		System.out.println("INFO: Character count: " + INVALID_NOTE.length());
		try {
			WebElement noteField = wait.until(ExpectedConditions.visibilityOfElementLocated(
				By.xpath("//div[text()='" + acname + "']/ancestor::div[@class='flex-1']/descendant::div[@data-placeholder='Add a note...']")));
			js.executeScript("arguments[0].scrollIntoView({behavior:'smooth',block:'center'});", noteField);
			Thread.sleep(800);
			noteField.click();
			Thread.sleep(500);
			noteField.sendKeys(Keys.CONTROL + "a");
			noteField.sendKeys(Keys.DELETE);
			Thread.sleep(300);
			noteField.sendKeys(INVALID_NOTE);
			Thread.sleep(1500);
			utilityclassobject.gettest().log(Status.INFO, "Entered 105 invalid characters in note field: '" + INVALID_NOTE + "'");
			System.out.println("INFO: Entered 105 invalid characters in note field successfully");
		} catch (Exception e) {
			utilityclassobject.gettest().log(Status.FAIL, "Failed to enter invalid note - " + e.getMessage());
			System.out.println("FAIL: Failed to enter invalid note - " + e.getMessage());
		}

		// Step 16: Delete the newly added payment card to clean up
		Thread.sleep(2000);
		try {
		...
		// Step 19: Wait for confirmation popup and click Delete button
		Thread.sleep(5000);
		...
		try {
			...
			if (confirmPopup != null) {
				...
				utilityclassobject.gettest().log(Status.FAIL, "Confirmation popup did not appear - skipping Delete");
