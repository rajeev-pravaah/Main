...
public class TC_019PastYear2021WithDifferentInput extends BaseClass80{
	ExcelUtility elib;
	javautility jlib;
	...
	@Test
	public void TC_019PastYear2021TextFieldWithInput() throws InterruptedException {
		...
		// Scroll into view
				...
				try {
					...
					WebElement target = bs.getTextfield2021();
					WebElement container = bs.getTextfield2021();
					...
					if (target.isEnabled() && target.isDisplayed()) {
						...
						utilityclassobject.gettest().log(Status.PASS, "Value added in 2021 text field and verified successfully: " );

						Thread.sleep(800);
						String got = (String) js.executeScript("return arguments[0].value;", target);
						if (valueToSet.equals(got)) {
							utilityclassobject.gettest().log(Status.PASS, "Entered 2021 value via sendKeys: " + got);
							// Print the actual data added into 2024 text field (single log line)
							utilityclassobject.gettest().log(Status.INFO, "Data added in 2021 text field: " + got);
							return;
						}
					}
				...
				try {
					...
					if (Boolean.TRUE.equals(contentEditable)) {
						...
						target.clear();
						utilityclassobject.gettest().log(Status.INFO, "Cleared 2021 field successfully");
						// Mention utility class object once after removing/clearing input
						utilityclassobject.gettest().log(Status.INFO, "utilityclassobject: 2021 field cleared");
					} else {
