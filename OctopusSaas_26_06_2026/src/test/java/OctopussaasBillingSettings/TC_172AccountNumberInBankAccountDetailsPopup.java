...
public class TC_172AccountNumberInBankAccountDetailsPopup extends BaseClass80 {
	ExcelUtility elib;
	javautility jlib;
	LoginPage lp;
	HomePage hp;
	...
	@Test
	public void TC_172AccountNumberInBankAccountDetailsPopup()
		...
		// 11d: Click "Provide bank account details" and detect popup
		...
		try {
			...
			utilityclassobject.gettest().log(Status.FAIL,
					"FAIL: Could NOT enter Routing Number - " + e.getMessage());
		}
		...
		// Fetch value from Excel - BillingSettings sheet, row 9, column 2
		// ======================================================
		String accountNum = elib.getDataFromExcel("BillingSettings", 9, 2);
		boolean accountEntered = false;

		try {
			// Stay in the same popup iframe context
			...
			utilityclassobject.gettest().log(Status.FAIL,
					"FAIL: Could NOT enter Account Number - " + e.getMessage());
		}
