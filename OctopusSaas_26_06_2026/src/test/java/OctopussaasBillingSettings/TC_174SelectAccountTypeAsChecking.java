...
public class TC_174SelectAccountTypeAsChecking extends BaseClass80 {
	ExcelUtility elib;
	javautility jlib;
	LoginPage lp;
	HomePage hp;
	...
	@Test
	public void TC_174SelectAccountTypeAsChecking()
		...
		// ======================================================
		// Step 14: Select "Checking" from accountSubType dropdown
		// ======================================================
		try {
			...
			// Verify "Checking" option exists in dropdown
			boolean checkingOptionExists = false;
			List<WebElement> allOptions = subTypeSelect.getOptions();
			for (WebElement option : allOptions) {
				if (option.getText().trim().equalsIgnoreCase("Checking")) {
					checkingOptionExists = true;
					break;
				}
			}

			if (checkingOptionExists) {
				utilityclassobject.gettest().log(Status.PASS,
						"'Checking' option is available in Account Sub Type dropdown");
				System.out.println("PASS: 'Checking' option is available in accountSubType dropdown");
			} else {
				utilityclassobject.gettest().log(Status.FAIL,
						"FAIL: 'Checking' option is NOT available in Account Sub Type dropdown");
				System.out.println("FAIL: 'Checking' option is NOT available in accountSubType dropdown");
			}

			// Select "Checking"
			subTypeSelect.selectByVisibleText("Checking");
			System.out.println("INFO: Selected 'Checking' from accountSubType dropdown");

			// Verify "Checking" is displayed as selected
			String selectedValue = subTypeSelect.getFirstSelectedOption().getText().trim();
			if (selectedValue.equalsIgnoreCase("Checking")) {
				utilityclassobject.gettest().log(Status.PASS,
						"'Checking' is selected and displayed in Account Sub Type dropdown successfully");
				System.out.println("PASS: 'Checking' is selected and displayed in accountSubType dropdown successfully");
			} else {
				utilityclassobject.gettest().log(Status.FAIL,
						"FAIL: Expected 'Checking' but displayed value is '" + selectedValue + "'");
				System.out.println("FAIL: Expected 'Checking' but displayed value is '" + selectedValue + "'");
			}
		} catch (Exception e) {
			utilityclassobject.gettest().log(Status.FAIL,
					"FAIL: Could NOT select 'Checking' from accountSubType dropdown - " + e.getMessage());
			System.out.println("FAIL: Could NOT select 'Checking' from accountSubType dropdown - " + e.getMessage());
		}
