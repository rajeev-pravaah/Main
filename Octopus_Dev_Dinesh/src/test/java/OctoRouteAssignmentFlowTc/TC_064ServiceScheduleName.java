...
public class TC_064ServiceScheduleName extends Baseclass{
	ExcelUtility elib;
	javautility jlib;
	...
	@Test
	public void TC_064ScheduleserviceName () throws InterruptedException, EncryptedDocumentException, IOException
	{
    	...
    	ras.getAccesscode().sendKeys(Keys.CONTROL + "a");
    	ras.getAccesscode().sendKeys(Keys.DELETE);
        ras.DeliveryNOte(note);
    	...
    	utilityclassobject.gettest().log(Status.INFO, "Service instruction auto saved successfully");
    	ras.getAccesscode().sendKeys(Keys.CONTROL + "a");
    	ras.getAccesscode().sendKeys(Keys.DELETE);
    	String accesscode1 = elib.getDataFromExcel("Routeassignment",4,4);
    	ras.AccessCode(accesscode1);
           String txt = ras.ServiceSchedulName();
