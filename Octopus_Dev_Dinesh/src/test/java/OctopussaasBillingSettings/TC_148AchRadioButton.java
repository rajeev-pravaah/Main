...
public class TC_148AchRadioButton extends BaseClass80 {
	...
	@Test
	public void TC_148AchRadioButton() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		// XPath: //label[text()='Account Holder Name *']/../descendant::input
		...
		// XPath: //span[text()='ACH']/../descendant::div
		// If already selected - leave as it is, otherwise click
		try {
			WebElement achRadioBtn = wait.until(
					ExpectedConditions.visibilityOf(bs.getACHradiobtn()));
			((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView(true);", achRadioBtn);
			Thread.sleep(1000);

			// Check if ACH radio button is already selected
			String ariaChecked = achRadioBtn.getAttribute("aria-checked");
			String classAttr   = achRadioBtn.getAttribute("class");
			boolean isSelected = achRadioBtn.isSelected()
			...
			if (isSelected) {
				utilityclassobject.gettest().log(Status.PASS, "ACH radio button is already selected - no action needed");
				System.out.println("PASS: ACH radio button is already selected - no action needed");
			} else {
				// Try normal click first, fallback to JS click
				try {
					achRadioBtn.click();
				} catch (Exception ex) {
					((JavascriptExecutor) driver).executeScript("arguments[0].click();", achRadioBtn);
				}
				Thread.sleep(1000);
				utilityclassobject.gettest().log(Status.PASS, "Clicked on ACH radio button successfully");
				System.out.println("PASS: Clicked on ACH radio button successfully");
			}
		} catch (Exception e) {
			utilityclassobject.gettest().log(Status.FAIL, "Failed to click ACH radio button - " + e.getMessage());
			System.out.println("FAIL: Failed to click ACH radio button - " + e.getMessage());
		}
	}
}
