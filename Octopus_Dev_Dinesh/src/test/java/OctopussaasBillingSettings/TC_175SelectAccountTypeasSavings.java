...
public class TC_175SelectAccountTypeasSavings extends BaseClass80 {
	ExcelUtility elib;
	javautility jlib;
	LoginPage lp;
	HomePage hp;
	...
	@Test
	public void TC_175SelectAccountTypeasSavings()
		...
		// Fetch from Excel - BillingSettings sheet, row 9, column 1
		// ======================================================
		...
		try {
			...
			if (val != null && !val.trim().isEmpty()) {
				...
				System.out.println("PASS: Entered Routing Number '" + routeNum + "' in popup successfully");
			}
		} catch (Exception e) {
			utilityclassobject.gettest().log(Status.FAIL, "FAIL: Could NOT enter Routing Number - " + e.getMessage());
			System.out.println("FAIL: Could NOT enter Routing Number - " + e.getMessage());
		}

		if (!routingEntered) {
			utilityclassobject.gettest().log(Status.FAIL, "FAIL: Could NOT enter Routing Number in popup");
			System.out.println("FAIL: Could NOT enter Routing Number in popup");
		}
		...
		System.out.println("INFO: Account Number to enter: '" + accountNum + "'");
		...
		try {
			...
			if (val != null && !val.trim().isEmpty()) {
				...
				System.out.println("PASS: Entered Account Number '" + accountNum + "' in popup successfully");
			}
		} catch (Exception e) {
			utilityclassobject.gettest().log(Status.FAIL, "FAIL: Could NOT enter Account Number - " + e.getMessage());
			System.out.println("FAIL: Could NOT enter Account Number - " + e.getMessage());
		}

		if (!accountEntered) {
			utilityclassobject.gettest().log(Status.FAIL, "FAIL: Could NOT enter Account Number in popup");
			System.out.println("FAIL: Could NOT enter Account Number in popup");
		}

		// ======================================================
		// Step 14: Select "Savings" from accountSubType dropdown
		// and verify it is displayed as selected
		// ======================================================
		try {
			...
			// Log default value before selection
			...
			System.out.println("INFO: Default value of accountSubType dropdown: '" + defaultValue + "'");

			// Verify "Savings" option exists in dropdown
			boolean savingsOptionExists = false;
			List<WebElement> allOptions = subTypeSelect.getOptions();
			System.out.println("INFO: All options in accountSubType dropdown:");
			for (WebElement option : allOptions) {
				String optionText = option.getText().trim();
				System.out.println("   - '" + optionText + "'");
				if (optionText.equalsIgnoreCase("Savings")) {
					savingsOptionExists = true;
				}
			}

			if (savingsOptionExists) {
				utilityclassobject.gettest().log(Status.PASS,
						"'Savings' option is available in Account Sub Type dropdown");
				System.out.println("PASS: 'Savings' option is available in accountSubType dropdown");
			} else {
				utilityclassobject.gettest().log(Status.FAIL,
						"FAIL: 'Savings' option is NOT available in Account Sub Type dropdown");
				System.out.println("FAIL: 'Savings' option is NOT available in accountSubType dropdown");
			}

			// Select "Savings"
			subTypeSelect.selectByVisibleText("Savings");
			System.out.println("INFO: Selected 'Savings' from accountSubType dropdown");

			// Verify "Savings" is displayed as selected
			String selectedValue = subTypeSelect.getFirstSelectedOption().getText().trim();
			if (selectedValue.equalsIgnoreCase("Savings")) {
				utilityclassobject.gettest().log(Status.PASS,
						"'Savings' is selected and displayed in Account Sub Type dropdown successfully");
				System.out.println("PASS: 'Savings' is selected and displayed in accountSubType dropdown successfully");
			} else {
				utilityclassobject.gettest().log(Status.FAIL,
						"FAIL: Expected 'Savings' but displayed value is '" + selectedValue + "'");
				System.out.println("FAIL: Expected 'Savings' but displayed value is '" + selectedValue + "'");
			}
		} catch (Exception e) {
			utilityclassobject.gettest().log(Status.FAIL,
					"FAIL: Could NOT select 'Savings' from accountSubType dropdown - " + e.getMessage());
			System.out.println("FAIL: Could NOT select 'Savings' from accountSubType dropdown - " + e.getMessage());
		}
