...
public class TC_069PaymentTermsWith9thelemtDueOnelement extends BaseClass80{
	ExcelUtility elib;
	javautility jlib;
	...
	@Test
	public void TC_069PaymentTermsDDwitheNinthelementDueonRecipt() throws InterruptedException {
		...
		// Verify that the 9th element is "Due on Recipt"
			if (options.size() >= 9) {
		    String ninthOptionText = options.get(8).getText(); // Index is 0-based
		    System.out.println("9th Option Text: " + ninthOptionText);
		    utilityclassobject.gettest().log(Status.INFO, "9th option text: " + ninthOptionText);
		    if (ninthOptionText.contains("Due on Receipt")) {
		        System.out.println("Verification Passed: 9th element is 'Due on receipt'");
		        utilityclassobject.gettest().log(Status.PASS, "9th element is 'Due on receipt'");
		    } else {
		        System.out.println("Verification Failed: 9th element is not 'Due on receipt'");
		        utilityclassobject.gettest().log(Status.FAIL, "9th element is not 'Due on receipt'");
		    }
		} else {
		    System.out.println("Verification Failed: Less than 9 options available in the dropdown");
		    utilityclassobject.gettest().log(Status.FAIL, "Less than 9 options available in the dropdown");
		}
	}
}
