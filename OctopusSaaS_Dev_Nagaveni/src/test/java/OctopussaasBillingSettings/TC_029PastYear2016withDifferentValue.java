...
public class TC_029PastYear2016withDifferentValue extends BaseClass80{
	ExcelUtility elib;
	javautility jlib;
	...
	@Test
	public void TC_029PastYear2016TextFieldWithInput() throws InterruptedException {
		...
		// Scroll into view
				...
				try {
					...
					WebElement target = bs.getTextfield2016();
					WebElement container = bs.getTextfield2016();
					...
					if (target.isEnabled() && target.isDisplayed()) {
						...
						utilityclassobject.gettest().log(Status.PASS, "Value added in 2016 text field and verified successfully: " );

						Thread.sleep(800);
						String got = (String) js.executeScript("return arguments[0].value;", target);
						if (valueToSet.equals(got)) {
							utilityclassobject.gettest().log(Status.PASS, "Entered 2016 value via sendKeys: " + got);
							utilityclassobject.gettest().log(Status.INFO, "Data added in 2016 text field: " + got);
							return;
						}
					}
				...
				try {
					...
					if (Boolean.TRUE.equals(contentEditable)) {
						...
						target.clear();
						utilityclassobject.gettest().log(Status.INFO, "Cleared 2016 field successfully");
						// Mention utility class object once after removing/clearing input
						utilityclassobject.gettest().log(Status.INFO, "utilityclassobject: 2016 field cleared");
					} else {
