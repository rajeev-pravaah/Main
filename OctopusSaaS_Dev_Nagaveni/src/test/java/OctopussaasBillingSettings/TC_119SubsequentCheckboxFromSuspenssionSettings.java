...
public class TC_119SubsequentCheckboxFromSuspenssionSettings extends BaseClass80 {
	...
	@Test
	public void TC_119SubsequentCheckboxFromSuspenssionSettings() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		// Perform click action on Subsequent Checkbox from Suspension Settings
		...
		String checkedState = subsequentCheckbox.getAttribute("class");
		if (checkedState != null && (checkedState.contains("checked") || checkedState.contains("active")
			...
			System.out.println("Subsequent Checkbox from Suspension Settings is CHECKED");
			utilityclassobject.gettest().log(Status.PASS, "Subsequent Checkbox from Suspension Settings is CHECKED successfully");
		} else {
			// Fallback to JavaScript click
			((JavascriptExecutor) driver).executeScript("arguments[0].click();", subsequentCheckbox);
			Thread.sleep(2000);
			System.out.println("Subsequent Checkbox from Suspension Settings clicked via JavaScript");
			utilityclassobject.gettest().log(Status.INFO, "Subsequent Checkbox from Suspension Settings clicked via JavaScript");
		}
	}
}
