...
public class TC_110AutmaticSuspensiondropdownwith6thvlause extends BaseClass80 {
	...
	@Test
	public void TC_110AutmaticSuspensiondropdownwith6thvlause() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		// Fetch 6th value
		if (allOptions != null && allOptions.size() >= 6) {
			String sixthValue = allOptions.get(5).getText().trim();
			System.out.println("6th Value from dropdown: " + sixthValue);
			utilityclassobject.gettest().log(Status.INFO, "6th Value from dropdown: " + sixthValue);
		} else {
			System.out.println("6th option not found in the dropdown");
			utilityclassobject.gettest().log(Status.FAIL, "6th option not found in the dropdown");
		}
	}
}
