...
public class TC_113AutmaticSuspensiondropdownwith9thvlause extends BaseClass80 {
	...
	@Test
	public void TC_113AutmaticSuspensiondropdownwith9thvlause() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		// Fetch 9th value
		if (allOptions != null && allOptions.size() >= 9) {
			String ninthValue = allOptions.get(8).getText().trim();
			System.out.println("9th Value from dropdown: " + ninthValue);
			utilityclassobject.gettest().log(Status.INFO, "9th Value from dropdown: " + ninthValue);
		} else {
			System.out.println("9th option not found in the dropdown");
			utilityclassobject.gettest().log(Status.FAIL, "9th option not found in the dropdown");
		}
	}
}
