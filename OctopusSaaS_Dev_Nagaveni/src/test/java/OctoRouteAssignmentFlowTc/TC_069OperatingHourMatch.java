package OctoRouteAssignmentFlowTc;

import java.awt.AWTException;
import java.io.IOException;
import java.time.LocalDate;
import java.util.ArrayList;
import java.util.List;

import org.apache.poi.EncryptedDocumentException;
import org.openqa.selenium.By;
import org.openqa.selenium.WebElement;
import org.testng.Assert;
import org.testng.annotations.Listeners;
import org.testng.annotations.Test;

import com.Octopussaas.BaseUtility.Baseclass;
import com.Octopussaas.FileUtility.ExcelUtility;
import com.Octopussaas.ObjectRepository.GeneratorManagentPage;
import com.Octopussaas.ObjectRepository.GeneretorInformation;
import com.Octopussaas.ObjectRepository.HomePage;
import com.Octopussaas.ObjectRepository.LoginPage;
import com.Octopussaas.ObjectRepository.RouteAssignment;
import com.Octopussaass.WebdriverUtility.javautility;
import com.Octopussaass.WebdriverUtility.utilityclassobject;
import com.Octopussaass.WebdriverUtility.webDriverutility;
import com.aventstack.extentreports.Status;
@Listeners(ListnerUtility.ListnerUilityImp.class)

public class TC_069OperatingHourMatch extends Baseclass{
	ExcelUtility elib;
	javautility jlib;
	webDriverutility wlib;

	LoginPage lp;
	HomePage hp;
	RouteAssignment ras;
	GeneratorManagentPage gmp;
	GeneretorInformation gip;
	
	
	@Test
	public void TC_069OperatingHourMatch() throws InterruptedException, EncryptedDocumentException, IOException, AWTException
	{
		hp=new HomePage(driver);
		elib=new ExcelUtility();
		hp.getGeneratoemanag().click();
        gmp = new GeneratorManagentPage(driver);
    	String gname = elib.getDataFromExcel("Generator", 1, 1);
		gmp.CharlieAccounttwo(gname);
        ras=new RouteAssignment(driver);
         gip = new GeneretorInformation(driver);
         gip.Ellisebtn();
     	utilityclassobject.gettest().log(Status.INFO, "Route assignment page is displayed successfully");  	
     	Thread.sleep(15000);
    	ras.AddService();
    	Thread.sleep(5000);
    	utilityclassobject.gettest().log(Status.INFO, "Add service displayed all text field successfully");
    	ras.Route1();
    	utilityclassobject.gettest().log(Status.INFO, "Route selected successfully");
    	ras.ServiceFrequency();
    	utilityclassobject.gettest().log(Status.INFO, "Service frequency selected successfully");
    	ras.SelectWeekDay();
    	ras.getClickoutside().click();;
    	utilityclassobject.gettest().log(Status.INFO, "week days selected successfully");
    	ras.getCalendar().click();
        LocalDate today = LocalDate.now();
        String day = String.valueOf(today.getDayOfMonth());
        driver.findElement(By.xpath("//*[text()='"+day+"']")).click();
    	utilityclassobject.gettest().log(Status.INFO, "Current date selected successfully");
    	ras.ServiceType();
    	utilityclassobject.gettest().log(Status.INFO, "Service type selected successfully");
    	WebElement element = ras.getDefaultdisposalFacility();
    	if(element.isDisplayed()) {
    	    System.out.println("Default disposal facility Element is visible");
    	} else {
    	    System.out.println("Default disposal facility Element is not visible");
    	}
    	Assert.assertTrue(element.isDisplayed());
    	utilityclassobject.gettest().log(Status.INFO, "Default disposal facility displayed successfully");
    	ras.defaultdisposaldd();
    	utilityclassobject.gettest().log(Status.INFO, "Default disposal facility seleccted value successfully");
        ras.ServiceDuration();
    	utilityclassobject.gettest().log(Status.INFO, "Service duration selected successfully");
    	Thread.sleep(5000);
    	ras.ScopeOfWork();
    	utilityclassobject.gettest().log(Status.INFO, "Scope of work selected successfully");
        Thread.sleep(2000);
        ras.IncreaseGalone();
    	ras.IncreaseGaltwo();
    	utilityclassobject.gettest().log(Status.INFO, "Scope of work increased successfully");
    	ras.AddRoute();
    	utilityclassobject.gettest().log(Status.INFO, "Service is adde to route successfully");
    	Thread.sleep(15000);
    	ras.CopyToclipBoard();
    	List<WebElement> operatingHours = driver.findElements(By.xpath("//div[contains(@class,'flex items-center') and contains(@class,'border-b')]/div[3]"));
    	String hours = "";

		for (WebElement ele : operatingHours) {
		    hours = ele.getText().trim();
		    System.out.println("Operating Hours: " + hours);
		}
    	utilityclassobject.gettest().log(Status.INFO, "Added date and timings fetch successfully");

	    	ras.RemoveServiceSchedule();
	    	utilityclassobject.gettest().log(Status.INFO, "Service is removed succesfully");
	    	ras.MoveTogeneratorInfo();
	    	ras.MoveTosundsay();
	    	
	    	
	    	List<WebElement> timings = driver.findElements(By.xpath("//input[@type='text' and contains(@placeholder,'Opening Time')]"));
	    	String value;
	    	for (WebElement time : timings) {
	    	    value = time.getAttribute("value");
	    	    System.out.println(value);
	    	    Assert.assertTrue(hours.contains(value),
	    	            "❌ " + value + " not found in " + hours);

	    	        System.out.println("✅ " + value + " found in " + hours);
	    	}
	    	utilityclassobject.gettest().log(Status.INFO, "Open hour verified and matched  successfully");
	    	List<WebElement> timings2 = driver.findElements(By.xpath("//input[@type='text' and contains(@placeholder,'Lunch Start Time')]"));
	    	String value2;
	    	for (WebElement time : timings2) {
	    	    value2 = time.getAttribute("value");
	    	    System.out.println(value2);
	    	    Assert.assertTrue(hours.contains(value2),
	    	            "❌ " + value2 + " not found in " + hours);

	    	        System.out.println("✅ " + value2 + " found in " + hours);
	    	}
	    	utilityclassobject.gettest().log(Status.INFO, "Lunch start hour verified and matched  successfully");
	    	List<WebElement> timings3 = driver.findElements(By.xpath("//input[@type='text' and contains(@placeholder,'Lunch End Time')]"));
	    	String value3;
	    	for (WebElement time : timings3) {
	    	    value3 = time.getAttribute("value");
	    	    System.out.println(value3);
	    	    Assert.assertTrue(hours.contains(value3),
	    	            "❌ " + value3 + " not found in " + hours);

	    	        System.out.println("✅ " + value3 + " found in " + hours);
	    	}
	    	utilityclassobject.gettest().log(Status.INFO, "Lunch end hour verified and matched  successfully");
	    	List<WebElement> timings4 = driver.findElements(By.xpath("//input[@type='text' and contains(@placeholder,'Closing Time')]"));
	    	String value4;
	    	for (WebElement time : timings4) {
	    	    value4 = time.getAttribute("value");
	    	    System.out.println(value4);
	    	    Assert.assertTrue(hours.contains(value4),
	    	            "❌ " + value4 + " not found in " + hours);

	    	        System.out.println("✅ " + value4 + " found in " + hours);
	    	}
	    	utilityclassobject.gettest().log(Status.INFO, "Closing hour verified and matched  successfully");
	    	
	    	
	    
	    	    
	    	
}
}
