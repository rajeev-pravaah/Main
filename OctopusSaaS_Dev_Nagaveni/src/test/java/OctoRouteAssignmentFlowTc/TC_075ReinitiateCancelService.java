...
public class TC_075ReinitiateCancelService extends Baseclass{
	ExcelUtility elib;
	javautility jlib;
	...
	@Test
	public void TC_075() throws InterruptedException, EncryptedDocumentException, IOException, AWTException {
		...
		Thread.sleep(3000);
		WebElement cancelele = ras.getCancelService();
		Assert.assertTrue(cancelele.isDisplayed(), "Element is NOT visible");
		utilityclassobject.gettest().log(Status.INFO, "Cancel service button is displayed");
		//ras.CancelServiceBTn();
		Thread.sleep(3000);
		...
		WebElement cancelBtn = wait.until(
		        ExpectedConditions.visibilityOfElementLocated(
		                By.xpath("//button[normalize-space()='Cancel Service']")
		        )
		);

		// scroll to that element inside the table container
		JavascriptExecutor js = (JavascriptExecutor) driver;
		js.executeScript("arguments[0].scrollIntoView({block:'center', inline:'center'});", cancelBtn);
		// click after scrolling
		cancelBtn.click();	
		ras.getCancellationNotetxtarea().click();
		ras.getCancellationNotetxtarea().sendKeys("cancel this service");
		utilityclassobject.gettest().log(Status.INFO, "Service cancellation note added successfully");
		ras.getCancelthisservice().click();
		utilityclassobject.gettest().log(Status.INFO, "Service cancelled successfully");
		ras.getReinstateseervie().click();
		ras.getReinstateThisservice().click();
		utilityclassobject.gettest().log(Status.INFO, "Service Reinstate successfully");
