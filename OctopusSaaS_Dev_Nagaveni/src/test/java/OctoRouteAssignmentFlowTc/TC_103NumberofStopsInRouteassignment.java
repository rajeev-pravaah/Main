...
public class TC_103NumberofStopsInRouteassignment extends Baseclass{
	ExcelUtility elib;
	javautility jlib;
	...
	@Test
	public void TC_103NumberofStopsInRoute() throws InterruptedException, EncryptedDocumentException, IOException, AWTException, HeadlessException, UnsupportedFlavorException {
		...
		Thread.sleep(15000);
        ras.HistoricalmaniFesReport();	 
		Thread.sleep(3000);
		ras.ServiceInstruction("xyz");
		utilityclassobject.gettest().log(Status.INFO, "Service instruction added  successfully");
		ras.getDateclcikoutside().click();
		ras.DeliveryNOte("Delivery");
		ras.getDateclcikoutside().click();
		utilityclassobject.gettest().log(Status.INFO, "Driver note added  successfully");
		ras.AccessCode("12676");
		ras.getDateclcikoutside().click();
		utilityclassobject.gettest().log(Status.INFO, "Access code added ");
		ras.getParkingNote().sendKeys("parked");
		ras.getDateclcikoutside().click();
		utilityclassobject.gettest().log(Status.INFO, "Parking note added");
		Thread.sleep(5000);
    	...
    	js1.executeScript("arguments[0].scrollIntoView({inline:'center'});", nayanRoute);
    	...
    	WebElement routePanel = driver.findElement(
    	        By.xpath("//p[text()='Nayan Route']/ancestor::div[contains(@class,'border')]")
    	);

    	WebElement charlie = routePanel.findElement(
    	        By.xpath(".//p[contains(text(),'"+gname+"')]")
    	);
        ...
        Thread.sleep(5000);
        WebElement count = driver.findElement(By.xpath("//p[text()='"+gname+"']/ancestor::div[contains(@class,'cursor-pointer')]//*[name()='text']"));
        String routecount = count.getText();
        System.out.println(routecount);
    	utilityclassobject.gettest().log(Status.INFO, "Rounte count fetched successfully"+routecount+"");
        // driver.findElement(By.xpath("//p[text()='"+gname+"' and not(ancestor::div[contains(@class,'cursor-not-allowed')])]/ancestor::div[@class='text-center bg-logoutBtn rounded-md']/descendant::button")).click();

        driver.findElement(By.xpath("(//p[text()='"+gname+"']/../../../descendant::div)[12]")).click();

        
        String Del = ras.getDElinRouteoptimization().getText();
        System.out.println(Del);
        Thread.sleep(2000);
    	utilityclassobject.gettest().log(Status.INFO, "In Permnent service instruction DEL(Delivery Note) fetched successfully");
    	String prk = ras.getPrkinRouteOptimization().getText();
    	System.out.println(prk);
        Thread.sleep(2000);
    	utilityclassobject.gettest().log(Status.INFO, "Permnent service instruction PRK(parking note)fetched successfully");
        String acode = ras.getAccessCodeInRouteOptimization().getText();
        Thread.sleep(2000);
        System.out.println(acode);
    	utilityclassobject.gettest().log(Status.INFO, "Permnent service instruction Access code fetched successfully");
    	String srinst = ras.getServiceNoteInRoueoptimization().getText();
    	System.out.println(srinst);
    	utilityclassobject.gettest().log(Status.INFO, "Permnent service instruction Service Instruction fetched successfully");
    	Thread.sleep(5000);
    	ras.CancelServiceInRouteOptimization();
    	Thread.sleep(5000);
    	...
    	ras.getDeliveryNote().clear();
    	ras.getAccesscode().clear();
    	ras.getParkingNote().clear();
    	ras.getServiceInstructionTextsrea().clear();
	}
}
