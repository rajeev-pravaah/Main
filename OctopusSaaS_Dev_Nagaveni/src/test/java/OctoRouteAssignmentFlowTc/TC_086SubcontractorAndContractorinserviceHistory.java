...
public class TC_086SubcontractorAndContractorinserviceHistory extends Baseclass{
	ExcelUtility elib;
	javautility jlib;
	...
	@Test
	public void TC_086() throws InterruptedException, EncryptedDocumentException, IOException, AWTException, HeadlessException, UnsupportedFlavorException {
		...
		ras.ScopeOfWorkInSSR();
		utilityclassobject.gettest().log(Status.INFO, "Scope of work  added successfully");
		ras.getSendtosubcontractor().click();
		utilityclassobject.gettest().log(Status.INFO, "Sub contractor added");
		WebElement scrollArea = driver.findElement(By.xpath("//div[contains(@class,'overflow-x-auto')]"));
		((JavascriptExecutor) driver).executeScript("arguments[0].scrollLeft += 500;", scrollArea);
		WebElement contractor = driver.findElement(By.xpath("//p[contains(text(),'Contractor:')]/following-sibling::p"));
		System.out.println("Contractor Name: " + contractor.getText());
		utilityclassobject.gettest().log(Status.INFO, "Contractor name displayed successfully");
		WebElement subcontractor = driver.findElement(By.xpath("//p[contains(text(),'Subcontractor:')]/following-sibling::p"));
		System.out.println("Subcontractor Name: " + subcontractor.getText());
		utilityclassobject.gettest().log(Status.INFO, "Subcontractor name displayed successfully");
		Thread.sleep(4000);
		ras.getCancelSSR().click();
		ras.getCancelSSrNOTe().sendKeys("Cancel SSR");
		ras.getCancelRequestBtn().click();
		utilityclassobject.gettest().log(Status.INFO, "SSR is removed successfully");
		Thread.sleep(4000);
