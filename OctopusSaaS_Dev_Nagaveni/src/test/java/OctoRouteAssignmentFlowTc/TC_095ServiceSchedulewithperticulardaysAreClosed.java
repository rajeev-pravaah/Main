...
public class TC_095ServiceSchedulewithperticulardaysAreClosed  extends Baseclass{
	ExcelUtility elib;
	javautility jlib;
	...
	@Test
	public void TC_095closedofficehours() throws InterruptedException, EncryptedDocumentException, IOException, AWTException
	{
		...
		ras.CopyToclipBoard();
		List<WebElement> operatingHours = driver.findElements(
			    By.xpath("//div[contains(@class,'w-40 shrink-0 truncate')]//p")
			);

			for (WebElement hour : operatingHours) {
			    System.out.println(hour.getText());
				utilityclassobject.gettest().log(Status.INFO, "Operating hour displayed successfully ");
