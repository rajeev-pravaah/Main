...
public class TC_094CloseOfficehourInScheduleService extends Baseclass{
	ExcelUtility elib;
	javautility jlib;
	...
	@Test
	public void TC_094closedofficehours() throws InterruptedException, EncryptedDocumentException, IOException, AWTException
	{
    	...
    	List<WebElement> closeCheckboxes = driver.findElements(
    		...
    		System.out.println("Total close values: " + naValues.size());

    		for (int j = 0; j < naValues.size(); j++) {
    		    String value = naValues.get(j).getText();
    		    System.out.println("Value " + (j + 1) + ": " + value);
    		}
        		utilityclassobject.gettest().log(Status.INFO, "Successfully fetched all th 10 operating hours as we mention as we selected");
            	...
            	List<WebElement> closeCheckboxess = driver.findElements(
            		    By.xpath("//div[@role='checkbox']")
            		);

            		System.out.println("Total close checkboxes: " + closeCheckboxess.size());

            		for (int i = 0; i < closeCheckboxess.size(); i++) {
            		    WebElement checkbox = closeCheckboxess.get(i);
