...
public class TC_112AutmaticSuspensiondropdownwith8thvlause extends BaseClass80 {
	...
	@Test
	public void TC_112AutmaticSuspensiondropdownwith8thvlause() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		// Fetch 8th value
		if (allOptions != null && allOptions.size() >= 8) {
			String eighthValue = allOptions.get(7).getText().trim();
			System.out.println("8th Value from dropdown: " + eighthValue);
			utilityclassobject.gettest().log(Status.INFO, "8th Value from dropdown: " + eighthValue);
		} else {
			System.out.println("8th option not found in the dropdown");
			utilityclassobject.gettest().log(Status.FAIL, "8th option not found in the dropdown");
		}
	}
}
