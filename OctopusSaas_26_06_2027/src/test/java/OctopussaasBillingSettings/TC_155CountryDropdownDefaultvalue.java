...
public class TC_155CountryDropdownDefaultvalue extends BaseClass80 {
	...
	@Test
	public void TC_155DefaultValuefromCountryDropdow() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		// Step 14: Fetch default value from Country dropdown (inside iframe)
		try {
			driver.switchTo().defaultContent();
			Thread.sleep(1000);

			// Locators to try for the country dropdown
			By[] countryLocators = {
				By.id("country-input"),
				...
				By.xpath("//select[@data-elements-stable-field-name='cardCountry']")
			};

			WebElement countryDropdown = null;
			int totalFrames = driver.findElements(By.tagName("iframe")).size();
			System.out.println("INFO: Scanning " + totalFrames + " iframe(s) for Country dropdown");

			outerSearch:
			for (int i = 0; i < totalFrames; i++) {
				for (By locator : countryLocators) {
					try {
						driver.switchTo().defaultContent();
						driver.switchTo().frame(i);
						java.util.List<WebElement> found = driver.findElements(locator);
						if (!found.isEmpty()) {
							countryDropdown = found.get(0);
							System.out.println("INFO: Country dropdown found in iframe[" + i + "] using: " + locator);
							break outerSearch;
						}
			...
			if (countryDropdown == null) {
				...
				for (By locator : countryLocators) {
					...
					if (!found.isEmpty()) {
						countryDropdown = found.get(0);
						System.out.println("INFO: Country dropdown found in default content using: " + locator);
						break;
					}
				}
			}
			...
			if (countryDropdown == null) {
				throw new Exception("Country dropdown could not be located in any iframe or on the page.");
			}

			Select select = new Select(countryDropdown);
			String defaultCountry = select.getFirstSelectedOption().getText().trim();
			System.out.println("INFO: Default selected country in dropdown is: " + defaultCountry);
			utilityclassobject.gettest().log(Status.PASS, "Default selected country in dropdown is: " + defaultCountry);

			driver.switchTo().defaultContent();

		} catch (Exception e) {
			driver.switchTo().defaultContent();
			utilityclassobject.gettest().log(Status.FAIL, "Failed to fetch default value from Country dropdown - " + e.getMessage());
			System.out.println("FAIL: Failed to fetch default value from Country dropdown - " + e.getMessage());
		}
	}
}