...
public class TC_108AutmaticSuspensiondropdownwith4thvlause extends BaseClass80 {
	...
	@Test
	public void TC_108AutmaticSuspensiondropdownwith4thvlause() throws InterruptedException, EncryptedDocumentException, IOException {
		...
		// Fetch 4th value
		if (allOptions != null && allOptions.size() >= 4) {
			String fourthValue = allOptions.get(3).getText().trim();
			System.out.println("4th Value from dropdown: " + fourthValue);
			utilityclassobject.gettest().log(Status.INFO, "4th Value from dropdown: " + fourthValue);
		} else {
			System.out.println("4th option not found in the dropdown");
			utilityclassobject.gettest().log(Status.FAIL, "4th option not found in the dropdown");
		}
	}
}
